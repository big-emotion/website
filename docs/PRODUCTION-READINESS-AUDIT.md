...
> Next.js + Docker + Traefik** (ADR 0005) and **tag-triggered CI deploy** (ADR 0006) — it still assumes static export + Apache/PHP + rsync/`.htaccess`, and
...
windowMs: 60_000})` _before_ parsing the body, and `route.ts:41-47` makes the send
...
- **Magic-link auth crypto is production-grade** (`src/lib/magic-link.ts`,
  `src/lib/session.ts`): single-use `randomBytes(32)` tokens deleted _unconditionally_
...
- **P2 — `PORTAL_BASE_URL` falls back silently to the request origin.**
  ...
  attacker-controllable `Host`, mailing a poisoned link to the _legitimate_ provisioned
...
- No blocking gaps. The `v0.5.1` caveat — that the reduced-motion fallback was _richer_
...
- **P1 — `src/proxy.ts` guards a route that does not exist.** `(auth)/verify/route.ts:20-21`
  redirects a _successfully_ authenticated user to `/espace/${consumed.clientId}`. There
  ...
  covers it. _(Unchanged from `v0.5.1`.)_
- **P2 — Brand hex duplicated in the OG/icon generators.** `src/app/opengraph-image.tsx:18`
  ...
  from `--color-lemon`. Lift to one shared exported constant. _(Unchanged.)_
...
- **P1 — The `/espace` dead-end still has no test coverage.** Nothing asserts where
  ...
  the same finding as D4, carried here because it is invisible to the suite _by
  construction_. _(Unchanged from `v0.5.1`.)_
...
- **P1 — CI Actions are still pinned by mutable tag, not SHA.** **16 references** across
  ...
  is proven in-repo, just not applied. _(Unchanged across three audits.)_
- **P1 — No `.github/dependabot.yml`.** 1,280 dependencies, unmonitored. _(Unchanged
  across three audits.)_
- **P2 — 3 open advisories** (`pnpm audit`, 1,280 deps: 1 high / 1 moderate / 1 low):
  ...
  - **moderate** `postcss@8.4.31` (XSS via unescaped `</style>`) — transitive via
    ...
    resolution depends on a Next patch release. _(Unchanged.)_
