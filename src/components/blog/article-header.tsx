...
export function ArticleHeader({
  ...
  return (
    <header className="border-b-2 border-current/30 pb-8">
      ...
      <h1 className="font-display mt-2 text-[clamp(2rem,8vw,5rem)] [overflow-wrap:anywhere]">
        {title}
      </h1>
      {hasMetaLine && (
      ...
      {thesis && (
        <blockquote className="mt-8 max-w-prose -rotate-1 bg-[var(--blog-ink)] px-6 py-5 text-[var(--blog-surface)] shadow-[6px_6px_0_var(--blog-accent)]">
          <p className="text-[clamp(1.25rem,1.05rem+1.4vw,1.75rem)] leading-snug font-semibold">
            {thesis}
          </p>
        </blockquote>
      )}
