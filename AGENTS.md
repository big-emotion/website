...
- **The brand charter is `brand/BRAND.md`** — read it before writing anything a visitor sees (colour, type, copy, photography, layout rhythm). It transcribes the designer's `brand/big-emotion-brand-guidelines.pdf` (also exported page by page to `brand/pages/*.jpg`, because an agent can read an image and not a PDF) into rules that can be grepped and reviewed. The `bigemotion-brand` skill loads it on demand. Two of its rules are tests rather than prose: `src/app/globals.css.test.ts` fails the build if a palette hex drifts from the charter or a seventh colour appears, and `src/content/site.test.ts` fails if accented copy lands in a `font-display` slot (DEC-023 — the shipped BBH woff2 has an ASCII-only cmap). `BRAND.md` ends with the known charter-vs-site divergences; add to that table rather than editing the charter to match the code.
