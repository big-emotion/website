...
**Branching: feature work targets `develop`, releases are cut from `main`.** Both automated paths open their PRs against `develop` — Ferry via the `FERRY_INTEGRATION_BRANCH` repo variable, `/bigemotion-ticket` via a hardcoded base — so branch from `origin/develop`, not `main`. `main` advances only when `develop` is promoted onto it, and only a `v*` tag on `main` deploys. Promotion is owned by a GitHub Actions workflow, `.github/workflows/promote-develop.yml`, that fast-forwards `main` to `develop` whenever CI is green there (ADR 0008) — rollout is pending because writing to `.github/` is outside what Ferry's code-writing agents are permitted to do, so a maintainer still has to add the workflow file by hand per the ADR before it runs automatically. Until it's live, promotion stays the same manual fast-forward it always was, and `/bigemotion-release` precondition 6 keeps refusing to tag while `develop` has commits `main` lacks — that check is not weakened by ADR 0008, only expected to fire less once the workflow is rolled out. `ci.yml` gates pushes to both branches and PRs into either.
