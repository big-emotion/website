...
**Legal pages and consent (SWBE-34).** Three routes — `/mentions-legales/`, `/politique-de-confidentialite/`, `/accessibilite/` — in both locales, linked from a row in the site footer alongside a "Gestion des cookies" button. They are `legal_page` Prismic documents rendered by `src/components/legal/`, but **Prismic is not allowed to take them down**: `legal-body.ts` falls back to the mandatory copy in `src/content/legal.ts` whenever the document is missing or its body is below a minimum floor, because LCEN art. 6-III and RGPD art. 13 do not pause while the CMS is empty. `src/content/legal.ts` also holds the registered company facts (SIREN 983 423 351, verified against the RNE) — deliberately not the president's home address, which the registry carries and the law does not ask for. The French copy is tutoiement like everything else, enforced by `src/content/tutoiement.test.tsx`; legal French is vouvoiement by convention, so this is the one place the register slips without anyone noticing.

**The site loads no third-party script.** `<PrismicPreview>` used to inject `static.cdn.prismic.io/prismic.js` on every page for every visitor; `src/components/prismic/prismic-toolbar.tsx` now mounts it only inside a draft-mode session, and keeps a small bootstrap that still turns an editor's preview cookie into a draft-mode session. **Consent** is tarteaucitron (vendored in `public/tarteaucitron/`, MIT), loaded **on demand** from the footer button — correct only while nothing needs consent before it runs. Adding the first consent-gated script (analytics, an embed, a pixel) means registering it in `CONSENT_SERVICES` _and_ switching `consent-manager.ts` to an eager load, or it will run before anyone has agreed to it. `consent-manager.test.ts` fails if a service is registered while the on-demand load is still in place, and checks the configured cookie name still matches the one the privacy policy names.
