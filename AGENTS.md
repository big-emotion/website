...
**Prismic (SWBE-24) — git is the source of truth for the content model.** Case studies are the one content type that has left `site.ts`: `case_study` documents feed `/cases` (the sector cards) and `/cases/[uid]` (the detail pages), fetched at **build time only** — both routes are `export const dynamic = "force-static"`, so **publishing in Prismic changes nothing until the site is rebuilt and redeployed**. That is the deliberate trade-off for keeping the marketing pages fully pre-rendered; a webhook→Actions rebuild is the follow-up when editors want faster turnaround.
...
**Branching: feature work targets `develop`, releases are cut from `main`.** Both automated paths open their PRs against `develop` — Ferry via the `FERRY_INTEGRATION_BRANCH` repo variable, `/bigemotion-ticket` via a hardcoded base — so branch from `origin/develop`, not `main`. `main` advances only when `develop` is promoted onto it, and only a `v*` tag on `main` deploys. The promotion is a deliberate manual step and currently has no owner: work merged to `develop` sits there until someone fast-forwards `main`, and `/bigemotion-release` refuses to tag while `develop` has commits `main` lacks. `ci.yml` gates pushes to both branches and PRs into either.
